...
build() {
  filename=$1
  go build -o ./${filename} -v -trimpath -ldflags "-X 'github.com/yaotthaha/cdns/constant.Commit=${COMMIT}' -X 'github.com/yaotthaha/cdns/constant.Version=${VERSION}' -s -w -buildid=" .
...
build_windows() {
  filename=$1
  go build -o ./${filename}.exe -v -trimpath -ldflags "-X 'github.com/yaotthaha/cdns/constant.Commit=${COMMIT}' -X 'github.com/yaotthaha/cdns/constant.Version=${VERSION}' -s -w -buildid=" .
  tar -czf ./build/${filename}.tar.gz ./${filename}.exe ./LICENSE ./README.md
  rm -rf ./${filename}.exe
  sha256sum ./build/${filename}.tar.gz > ./build/${filename}.tar.gz.sha256
  echo "Build $filename OK!!"
}
...
GOARCH=386 GOOS=freebsd build "${NAME}-freebsd-386"
GOARCH=amd64 GOOS=freebsd build "${NAME}-freebsd-amd64"
GOARCH=amd64 GOOS=freebsd GOAMD64=v3 build "${NAME}-freebsd-amd64-v3"
GOARCH=arm64 GOOS=freebsd build "${NAME}-freebsd-arm64"
